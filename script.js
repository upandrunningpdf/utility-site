const units = {
  ...
  volume: {
    ...
    units: {
      ...
      imperialQuart: { name: "Imperial Quart", factor: 1.1365225 },
      imperialPint: { name: "Imperial Pint", factor: 0.56826125 },
      imperialCup: { name: "Imperial Cup", factor: 0.284130625 },
      imperialFluidOunce: { name: "Imperial Fluid Ounce", factor: 0.0284130625 }
    }
  },
  ...
  speed: {
    ...
    units: {
      ...
      kilometerPerHour: { name: "Kilometers per Hour", factor: 0.2777777777777778 },
      milePerHour: { name: "Miles per Hour", factor: 0.44704 },
      footPerSecond: { name: "Feet per Second", factor: 0.3048 },
      knot: { name: "Knots", factor: 0.5144444444444445 }
    }
  },
  ...
  energy: {
    ...
    units: {
      ...
      electronvolt: { name: "Electronvolt", factor: 1.602176634e-19 }
    }
  },
  ...
  power: {
    ...
    units: {
      ...
      horsepower: { name: "Horsepower", factor: 745.6998715822702 },
      btuPerHour: { name: "BTU per Hour", factor: 0.2930710701722222 }
    }
  },
  ...
  cooking: {
    ...
    units: {
      ...
      fluidOunce: { name: "US Fluid Ounce", factor: 29.5735295625 },
      cup: { name: "US Cup", factor: 236.5882365 },
      pint: { name: "US Pint", factor: 473.176473 },
      quart: { name: "US Quart", factor: 946.352946 },
      gallon: { name: "US Gallon", factor: 3785.411784 },
...
function calculate() {
  ...
  if (category.special) {
    ...
    result = baseValue / category.units[to].factor;
  }
...
function formatNumber(number) {
  ...
  return Number(number.toPrecision(12)).toLocaleString(
    undefined,
    {
...
copyButton.addEventListener("click", async () => {
  ...
  if (Number.isNaN(value)) {
    copyMessage.textContent = "Enter a value first.";
    return;
  }
  ...
  const fromName =
    units[categorySelect.value].units[fromUnitSelect.value].name;

  const toName =
    units[categorySelect.value].units[toUnitSelect.value].name;

  const text = `${formatNumber(value)} ${fromName} = ${result} ${toName}`;

  try {
    await navigator.clipboard.writeText(text);
    copyMessage.textContent = "Copied!";
  } catch {
    copyMessage.textContent = "Unable to copy automatically.";
  }
...
const popularConversions = {
  "miles-kilometers": ["length", "mile", "kilometer"],
  "kilometers-miles": ["length", "kilometer", "mile"],
  "pounds-kilograms": ["mass", "pound", "kilogram"],
  "kilograms-pounds": ["mass", "kilogram", "pound"],
  "feet-meters": ["length", "foot", "meter"],
  "meters-feet": ["length", "meter", "foot"],
  "fahrenheit-celsius": ["temperature", "fahrenheit", "celsius"],
  "celsius-fahrenheit": ["temperature", "celsius", "fahrenheit"],
  "cups-milliliters": ["cooking", "cup", "milliliter"],
  "liters-gallons": ["volume", "liter", "usGallon"]
};
...
document.querySelectorAll("[data-conversion]").forEach(button => {
  ...
  button.addEventListener("click", () => {
    ...
    const conversion =
      popularConversions[button.dataset.conversion];

    if (!conversion) return;

    categorySelect.value = conversion[0];

    loadUnits();

    fromUnitSelect.value = conversion[1];
    toUnitSelect.value = conversion[2];
